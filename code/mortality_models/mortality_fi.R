...
# randomize mortality within species and create new mort_next_rand column
dat_mort %>% 
  group_by(sp) %>%
  mutate(mort_next_rand = sample(mort_next, size = n())) %>% 
  as.data.frame() -> dat_mort

# check result
table(dat_mort$mort_next_rand, dat_mort$mort_next)

dat_mort %>% 
  group_by(sp) %>% 
  summarise(mort_rate = sum(mort_next)/n(),
            mort_rate_rand = sum(mort_next_rand)/n()) -> check_sim
plot(check_sim[, 2:3])
...
model_fit = function(data, speciesinfo) {
  ...
  form =  as.formula(paste0("mort_next_rand ~ s(dbh, k = k1) + s(all_BA, k = k2)  + s(con_BA, k = k3)"
                            , term_c))
